import { AIAnalysis } from '@bugrank/shared';
...
Return ONLY the JSON object, no additional text. No markdown. No code blocks.`;
  ...
  private parseGeminiResponse(text: string): AIAnalysis {
    try {
  ...
  private buildDetailedFeedback(parsed: any): string {
    let feedback = '';
    ...
    if (parsed.efficiencyScore || parsed.correctnessScore || parsed.codeQualityScore) {
      feedback += `\n\nScores:\n`;
      if (parsed.correctnessScore) feedback += `  * Correctness: ${parsed.correctnessScore}/10\n`;
      if (parsed.efficiencyScore) feedback += `  * Efficiency: ${parsed.efficiencyScore}/10\n`;
      if (parsed.codeQualityScore) feedback += `  * Code Quality: ${parsed.codeQualityScore}/10\n`;
    }
    ...
    if (parsed.optimizations && parsed.optimizations.length > 0) {
      ...
      parsed.optimizations.forEach((opt: string) => {
        feedback += `  * ${opt}\n`;
      });
    }
